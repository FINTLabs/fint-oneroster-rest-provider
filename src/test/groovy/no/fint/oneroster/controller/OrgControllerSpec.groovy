...
class OrgControllerSpec extends Specification {
    ...
    def "Given valid orgId return list of orgs"() {
        when:
        def response = mockMvc.perform(get("/orgs"))

        then:
        1 * orgService.getAllOrgs() >> getOrgs()
    ...
    def "Given fields parameter returns list of orgs with only selected fields"() {
        when:
        def response = mockMvc.perform(get("/orgs").param('fields', 'sourcedId'))
    ...
    def "Given offset parameter returns sublist of orgs"() {
        when:
        def response = mockMvc.perform(get("/orgs").param('offset', '0'))
    ...
    def "Given limit parameter returns sublist of orgs"() {
        when:
        def response = mockMvc.perform(get("/orgs").param('limit', '1'))
    ...
    def "Given sort parameter return sorted list of orgs"() {
        when:
        def response = mockMvc.perform(get("/orgs").param('sort', 'name'))
    ...
    def "Given filter parameter return sublist of orgs meeting criteria"() {
        when:
        def response = mockMvc.perform(get("/orgs").param('filter', 'name~\'b\''))
    ...
    def "Given valid sourcedId return org"() {
        when:
        def response = mockMvc.perform(get("/orgs/12"))

        then:
        1 * orgService.getOrg('12') >> getOrgs().first()
        response.andExpect(status().isOk())
    ...
    def "Given valid sourcedId and fields parameter return org with only selected fields"() {
        when:
        def response = mockMvc.perform(get("/orgs/12").param('fields', 'sourcedId'))
